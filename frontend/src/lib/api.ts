...
export interface Proposal { proposal_id:string;status:string;actions:Array<{action_id:string;payment_id:string;action_type:string;amount_subunits:number;rationale:string}>;total_amount_subunits:number;expires_at:string;policy_allowed:boolean;policy_version:string }
export interface AuditEvent { audit_id:string;event_type:string;occurred_at:string }
...
export const decideProposal=(id:string,action:'approve'|'reject')=>request<{decision:string}>(`/proposals/${id}/${action}`,{method:'POST',body:JSON.stringify({decided_by:'demo-merchant-operator'})})
